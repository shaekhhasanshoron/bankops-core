...
type TransactionSaga struct {
	...
	SuccessfulFinishedSteps string     `gorm:"type:text"` // json string
...
func (s *TransactionSaga) AppendSuccessfulStep(stepType string) error {
	steps := s.GetSuccessfulSteps()
	...
	return s.SetSuccessfulSteps(steps)
}

func (s *TransactionSaga) GetSuccessfulSteps() []string {
	if s.SuccessfulFinishedSteps == "" {
		return []string{}
	}

	var steps []string
	if err := json.Unmarshal([]byte(s.SuccessfulFinishedSteps), &steps); err != nil {
		return []string{}
	}
	return steps
}

func (s *TransactionSaga) SetSuccessfulSteps(steps []string) error {
	data, err := json.Marshal(steps)
	if err != nil {
		return err
	}
	s.SuccessfulFinishedSteps = string(data)
	return nil
}

func (s *TransactionSaga) IsStepsCompletedSuccessfully(currentSteps ...string) bool {
	count := 0
	steps := s.GetSuccessfulSteps()
	for _, currentStep := range currentSteps {
		for _, finishedStep := range steps {
