...
func TestLoadConfig_EnvOverride(t *testing.T) {
	_ = os.Setenv("TRANSACTION_HTTP__ADDR", ":9090")
	defer unset("TRANSACTION_HTTP__ADDR")
	cfg, err := LoadConfig()
	if err != nil {
		t.Fatalf("LoadConfig error: %v", err)
	}
...
TRANSACTION_GRPC__ADDR=:9090
TRANSACTION_HTTP__ADDR=:9091
`
...
func TestInjectFiles_ErrorFileNotFound(t *testing.T) {
	...
	originalEnv := os.Getenv("TRANSACTION_MISSING_FILE")
	defer os.Setenv("TRANSACTION_MISSING_FILE", originalEnv)

	os.Setenv("TRANSACTION_MISSING_FILE", "/non/existent/file")

	err := injectFiles(k, "TRANSACTION_", "_FILE")
	assert.NoError(t, err)
}
