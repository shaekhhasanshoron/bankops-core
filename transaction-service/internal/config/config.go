/*
	...
	Load order & precedence (lowest → highest):
		- defaults -> .env files (loaded when TRANSACTION_ENV != "prod") -> *_FILE file injection -> real environment variables

	Env naming conventions:
		- Use a service-specific prefix: TRANSACTION_.
		- Use double underscores "__" to denote nesting. e.g. TRANSACTION_HTTP__ADDR=":8080"  -> http.addr
	Files Paths (*_FILE):
...
func LoadConfig(envFiles ...string) (*Config, error) {
	...
	// Setting default env
	appEnv := strings.ToLower(strings.TrimSpace(os.Getenv("TRANSACTION_ENV")))
	if appEnv == "" {
		appEnv = EnvProd
	}
	...
	if appEnv != EnvProd {
		...
		appEnvOverwrite := strings.ToLower(strings.TrimSpace(os.Getenv("TRANSACTION_ENV")))
		if appEnvOverwrite != "" {
...
// injectFiles reads TRANSACTION_*_FILE envs and sets their contents to the corresponding key.
// e.g. TRANSACTION_DB__PASSWORD_FILE=/config/secret/data → (get data from file /config/secret/data) -> sets "db.password"  (if TRANSACTION_DB__PASSWORD is not set in env directly).
func injectFiles(k *koanf.Koanf, prefix, suffix string) error {
	...
	for _, e := range os.Environ() {
		...
		envKey, filePath := parts[0], parts[1] // e.g. envKey=TRANSACTION_DB__PASSWORD_FILE; filePath=/config/secret/data

		if !strings.HasPrefix(envKey, prefix) || !strings.HasSuffix(envKey, suffix) {
			continue
		}

		base := strings.TrimSuffix(envKey, suffix) // e.g. TRANSACTION_DB__PASSWORD_FILE -> TRANSACTION_DB__PASSWORD
		...
		// maps env -> koanf key; e.g. TRANSACTION_DB__PASSWORD -> db.password
